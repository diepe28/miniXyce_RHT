...
//////////////////// distributed_sparse_matrix_add_to /////////////////////////
...
void mX_matrix_utils::distributed_sparse_matrix_add_to_producer(distributed_sparse_matrix* M, int row_idx, int col_idx, double val, int n, int p) {
    // implements M[row_idx][col_idx] += val
    // man, in the distributed matrix world, this simple thing takes such a lot of effort!
...
void mX_matrix_utils::distributed_sparse_matrix_add_to_consumer(distributed_sparse_matrix* M, int row_idx, int col_idx, double val, int n, int p) {
    // implements M[row_idx][col_idx] += val
    // man, in the distributed matrix world, this simple thing takes such a lot of effort!
    ...
    if ((row_idx >= M->start_row) && (row_idx <= M->end_row)) {
        ...
        bool inserted = false;
        /*-- RHT -- */ RHT_Consume_Check(inserted);
        ...
        while ((curr) && (!inserted)) {
            if (curr->column < col_idx) {
                ...
                if (curr->column == col_idx) {
                    curr->value = curr->value + val;
                    /*-- RHT -- */ RHT_Consume_Check(curr->value);
                    inserted = true;
                    /*-- RHT -- */ RHT_Consume_Check(inserted);
                } else {
                    distributed_sparse_matrix_entry *entry_ptr_1 = new distributed_sparse_matrix_entry();
    ...
    if ((col_idx >= M->start_row) && (col_idx <= M->end_row)) {
        ...
        int pid_to_send_info;
        bool pid_found = false;
        /*-- RHT -- */ RHT_Consume_Check(pid_found);

        int start_pid = 0;
        int end_pid = p - 1;
        int mid_pid = (start_pid + end_pid) / 2;
        /*-- RHT -- */ RHT_Consume_Check(start_pid);
        /*-- RHT -- */ RHT_Consume_Check(end_pid);
        /*-- RHT -- */ RHT_Consume_Check(mid_pid);
        ...
        while (!pid_found) {
            if (row_idx < mid_start_row) {
                ...
                if (row_idx > mid_end_row) {
                    ...
                    pid_to_send_info = mid_pid;
                    pid_found = true;
                    /*-- RHT -- */ RHT_Consume_Check(pid_to_send_info);
                    /*-- RHT -- */ RHT_Consume_Check(pid_found);
                }
            }
        ...
        if (!send_instruction_posted) {
            data_transfer_instruction *dti_ptr_1 = new data_transfer_instruction();
            dti_ptr_1->pid = pid_to_send_info;
            /*-- RHT -- */ RHT_Consume_Check(dti_ptr_1->pid);
            /*-- RHT -- */ RHT_Consume_Check(col_idx);
            // is this a volatile access?
            dti_ptr_1->indices.push_back(col_idx);
...
double mX_matrix_utils::norm_producer(std::vector<double> &x) {
    // at last, a function that's relatively simple to implement in parallel
...
double mX_matrix_utils::norm_consumer(std::vector<double> &x) {
    // at last, a function that's relatively simple to implement in parallel
...
void mX_matrix_utils::gmres_producer(distributed_sparse_matrix* A, std::vector<double> &b, std::vector<double> &x0, double &tol, double &err, int k, std::vector<double> &x, int &iters, int &restarts) {
    // here's the star of the show, the guest of honor, none other than Mr.GMRES
    ...
    while (err > tol) {
        ...
        for (int i = start_row; i <= end_row; i++) {
            ...
            /*-- RHT -- */ RHT_Produce(temp1[i - start_row]);
            std::vector<double> temp2;
            temp2.push_back(temp1[i - start_row]);
            V.push_back(temp2);
        }
        ...
        for (int i = start_row; i <= end_row; i++) {
            V[i - start_row][0] /= beta;
            /*-- RHT -- */ RHT_Produce(V[i - start_row][0]);
        }
        ...
        while ((err > tol) && (iters < k)) {
            ...
            for (int i = 0; i < iters; i++) {
                ...
                for (int j = start_row; j <= end_row; j++) {
                    temp2[j - start_row] -= global_dot * V[j - start_row][i];
                    /*-- RHT -- */ RHT_Produce(temp2[j - start_row]);
                }
            ...
            for (int i = start_row; i <= end_row; i++) {
                temp2[i - start_row] /= new_col_H.back();
                /*-- RHT -- */ RHT_Produce(temp2[i - start_row]);
                V[i - start_row].push_back(temp2[i - start_row]);
            }
...
void mX_matrix_utils::gmres_consumer(distributed_sparse_matrix* A, std::vector<double> &b, std::vector<double> &x0, double &tol, double &err, int k, std::vector<double> &x, int &iters, int &restarts) {
    ...
    for (int i = 0; i < temp1.size(); i++) {
        temp1[i] -= b[i];
        /*-- RHT -- */ RHT_Consume_Check(temp1[i]);
    }

    err = norm_consumer(temp1);
    ...
    while (err > tol) {
        ...
        for (int i = start_row; i <= end_row; i++) {
            ...
            /*-- RHT -- */ RHT_Consume_Check(temp1[i - start_row]);
            std::vector<double> temp2;
            temp2.push_back(temp1[i - start_row]);
            V.push_back(temp2);
        }

        double beta = norm_consumer(temp1);
        /*-- RHT -- */ RHT_Consume_Check(beta);

        for (int i = start_row; i <= end_row; i++) {
            V[i - start_row][0] /= beta;
            /*-- RHT -- */ RHT_Consume_Check(V[i - start_row][0]);
        }
        ...
        while ((err > tol) && (iters < k)) {
            ...
            for (int i = 0; i < iters; i++) {
                ...
                for (int j = start_row; j <= end_row; j++) {
                    temp2[j - start_row] -= global_dot * V[j - start_row][i];
                    /*-- RHT -- */ RHT_Consume_Check(temp2[j - start_row]);
                }

                new_col_H.push_back(global_dot);
            }

            double normTemp2 = norm_consumer(temp2);
            /*-- RHT -- */ RHT_Consume_Check(normTemp2);
            new_col_H.push_back(normTemp2);

            for (int i = start_row; i <= end_row; i++) {
                temp2[i - start_row] /= new_col_H.back();
                /*-- RHT -- */ RHT_Consume_Check(temp2[i - start_row]);
                V[i - start_row].push_back(temp2[i - start_row]);
            }
            ...
            for (int i = 0; i < iters - 1; i++) {
                ...
                new_col_H[i] = cosines[i] * old_i + sines[i] * old_i_plus_one;
                new_col_H[i + 1] = -sines[i] * old_i + cosines[i] * old_i_plus_one;
                /*-- RHT -- */ RHT_Consume_Check(new_col_H[i]);
                /*-- RHT -- */ RHT_Consume_Check(new_col_H[i + 1]);
            }
            ...
            double old_i = new_col_H[iters - 1];
            double old_i_plus_one = new_col_H[iters];
            /*-- RHT -- */ RHT_Consume_Check(old_i);
            /*-- RHT -- */ RHT_Consume_Check(old_i_plus_one);

            new_col_H[iters - 1] = cosines.back() * old_i + sines.back() * old_i_plus_one;
            /*-- RHT -- */ RHT_Consume_Check(new_col_H[iters - 1]);
            new_col_H.pop_back();
            ...
            double old_g = g[iters - 1];
            g[iters - 1] = old_g * cosines.back();
            /*-- RHT -- */ RHT_Consume_Check(old_g);
            /*-- RHT -- */ RHT_Consume_Check(g[iters - 1]);
            tempVar = -old_g * sines.back();
